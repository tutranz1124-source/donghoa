...
import { Search, X, ArrowRight, Building2, BookOpen, MapPin, Sparkles, Tag, Clock, ChevronRight } from 'lucide-react';
...
export default function HeaderSearchBar({ className = '', isMobileDrawer = false }: HeaderSearchBarProps) {
  ...
  const handleSelectProject = (project: ProjectItem) => {
    setIsOpen(false);
    // Smooth scroll to projects section or open modal
  ...
  return (
    <>
      ...
      {/* Full Luxury Command Palette Search Dialog */}
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-[100] flex items-start justify-center pt-12 sm:pt-20 px-4 sm:px-6 overflow-y-auto">
            {/* Backdrop */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            <motion.div
              ...
              role="dialog"
              aria-modal="true"
              className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden z-10 border border-warm-200 my-auto flex flex-col max-h-[85vh]"
            >
              {/* Search Bar Input Header */}
              <div className="relative flex items-center px-5 sm:px-6 py-4 border-b border-warm-200 bg-warm-50/70">
